...
import { IGridProps, IGridColumnProps, TGridSize } from './interfaces';
    ...
    if (!(stringifyArgs in memoizeBuffer)) {
      ...
      memoizeBuffer[stringifyArgs] = rows
        .map((row: TRow) => {
          ...
          // if alignItems === 'end' then empty columns are placed at the beginning of the line
          // otherwise empty columns are placed at the end of the line
...
function getTemplate({
  ...
  const customMediaQueries = resultBreakPointsEntries.map(([breakPoint, size]) => {
    const numberSize = size.gridTemplate as TGridSize;
