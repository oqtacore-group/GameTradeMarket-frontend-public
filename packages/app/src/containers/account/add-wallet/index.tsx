...
import { Modal, COLORS, Button } from '@game-trade/ui';
import { useCustomWalletConnectorProvider, TConnector } from '@game-trade/lib';
...
import {
  ...
  NetworkWalletList,
  NetworkWalletItem,
  NetworkWalletItemLabel,
} from './style';
// eslint-disable-next-line @typescript-eslint/ban-ts-comment
// @ts-ignore
import SvgMetamask from './metamask.svg';
import { AddWalletName } from './add-name';
...
export function AddWalletComponent(props: IProps) {
  ...
  useEffect(() => {
    ...
    if (account && !isActivating && isActive && !error) {
      const found = addressList.find((a) => a === account?.toLowerCase());
      if (!found) {
        setShowAddNameModal(true);
      } else {
        setShowAddNameModal(false);
        if (currentConnector === 'WalletConnect') {
          setNotificationModalMessage(`Address ${formatAddress(account as string)} already added`);
        }
      }
    }
  }, [isActivating, isActive, provider, connector, account, addressList, currentConnector]);

  useEffect(() => {
    if (ifFirstOpen || visible) {
      return;
    }
    if (hookWalletPhantom?.publicKey?.toString()) {
      setShowAddNameModal(true);
    }
  }, [hookWalletPhantom, visible]);

  // MetaMask | WalletConnect
  let clear: any = null;
  const handleConnect = useCallback(
    async (connector: TConnector | string) => {
        ...
        if (isActive && currentConnector === connector) {
          ...
          if (!found) {
            ...
            setNotificationModalMessage(
              `Address ${formatAddress(account as string)} already added`
            );
          }
        } else {
          await activateConnector(connector);
      ...
      if (connector === 'Phantom') {
        const found = addressList.find((a) => a === account?.toLowerCase());
        if (!hookWalletPhantom?.publicKey?.toString() && !found) {
          setVisible(true);
        } else if (!found) {
          setShowAddNameModal(true);
        } else {
    ...
    [isActive, provider, connector, account, addressList, hookWalletPhantom]
  );
  ...
  return (
    <Modal isCloseOutside={false} onClose={onClose} hasHeader={false} isPadding={false} size={560}>
      <ModalContent>
        <Header>
          {t('addWallet_2')}{' '}
          {isActive && (
            <span>
              (
              <span>
                {t('connected')} {currentConnector}
              </span>
              )
            </span>
          )}
        </Header>
        {!showAddNameModal && !notificationModalMessage && (
          <NetworkWalletList>
            {!isMobile && (
              <NetworkWalletItem
            ...
            {!isMobile && (
              <NetworkWalletItem
                ...
                isActive={Boolean(window.phantom?.solana)}>
                <NetworkWalletItemLabel>
                  <svg
                    width="37"
                    height="36"
        ...
        {showAddNameModal && (
          <AddWalletName
            ...
            onConfirm={onConfirm}
            account={account || hookWalletPhantom?.publicKey?.toString() || ''}
