...
import IconCheckPinkPng from '@root/public/imgs/roadmap/icon_check_pink.png';
...
export const ContentRow = styled.div<{ colStart?: number; rowStart?: number; isDone?: boolean }>`
  display: flex;
  align-items: center;
    ...
    ${(p) => {
      if (!p.isDone) {
        ...
        return `
          width: 15px;
          height: 15px;
          background-image: url(${IconCheckPinkPng.src});
