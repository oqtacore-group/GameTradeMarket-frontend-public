resource "aws_amplify_app" "GameTradeMarket-frontend-admin" {
  name       = var.name
  platform = "WEB_DYNAMIC"
  iam_service_role_arn = aws_iam_role.amplify_role_admin.arn
...
//  }
  ...
  environment_variables = {
    ...
    AMPLIFY_MONOREPO_APP_ROOT = "packages/admin"
...
resource "aws_amplify_branch" "release-admin" {
  app_id      = aws_amplify_app.GameTradeMarket-frontend-admin.id
  branch_name = "release"
...
resource "aws_amplify_domain_association" "GameTradeMarket-frontend-admin-domain" {
  ...
  sub_domain {
    branch_name = aws_amplify_branch.release-admin.branch_name
    prefix      = ""
  }
}

resource "aws_iam_role" "amplify_role_admin" {
  name = "amplify-role-admin"
...
data "aws_iam_policy" "AdministratorAccessAdmin" {
  arn = "arn:aws:iam::aws:policy/AdministratorAccess"
}

resource "aws_iam_role_policy_attachment" "sto-administrator-role-policy-attach" {
  role       = aws_iam_role.amplify_role_admin.name
  policy_arn = data.aws_iam_policy.AdministratorAccessAdmin.arn
}
